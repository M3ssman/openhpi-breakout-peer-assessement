...
//        Ball ball = mock(Ball.class);
        ...
        when(paddle.getGeometry()).thenReturn(new Ellipse2D.Float(0, 0, ballSize, ballSize));
        assertFalse("At this position ball should not intersect with brick.", this.paddle.intersects(paddle.getGeometry()));

        //put the mocked ball on the brick
        when(paddle.getGeometry()).thenReturn(new Ellipse2D.Float(Game.SCREEN_X / 2, Game.SCREEN_Y - 200, ballSize, ballSize));
        assertTrue("At this position ball should intersect with brick.", this.paddle.intersects(paddle.getGeometry()));
    }
}
